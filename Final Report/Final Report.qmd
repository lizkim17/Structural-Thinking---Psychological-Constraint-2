...
```
to check if Condition (Baseline, Between, Within) is associated with robot_OE_cat and puzzle_OE_cat.
...
```
To check whether different conditions, age, or gender influence the likelihood of providing a structural explanation.

```{r}
# Odds Ratio Calculations
exp(logistic.struc$coefficients['ConditionBetween'])
exp(logistic.struc$coefficients['ConditionWithin'])
```
The odds ratio tells us how much more or less likely children in Between and Within conditions are to give structural explanations, compared to Baseline.
