...
non_numeric_summary <- data %>%
  count(Condition) %>%
  mutate(Proportion = round(n / sum(n) * 100, 2)) %>%  # Round proportions for readability
  rename(
    "Condition" = Condition,
    "Count" = n,
...
results_table_ttest <- data.frame(
  "Comparison" = c("Baseline vs. Between", "Baseline vs. Within"),
  "t-value" = round(c(t_test_baseline_between$statistic, t_test_baseline_within$statistic), 3),
  "df" = c(t_test_baseline_between$parameter, t_test_baseline_within$parameter),
  "p-value" = signif(c(t_test_baseline_between$p.value, t_test_baseline_within$p.value), 3),
  "95% CI Lower" = round(c(t_test_baseline_between$conf.int[1], t_test_baseline_within$conf.int[1]), 2),
  "95% CI Upper" = round(c(t_test_baseline_between$conf.int[2], t_test_baseline_within$conf.int[2]), 2)
)
print(results_table_ttest)
```
...
data <- read_csv("ST-P 2 Dataset use this!!!.csv")  
data <- as.data.frame(data) #this was the only way to fix the error 
